...
namespace gym {
    ...
    template<bool cont>
    std::vector<double> LunarLandarEnv<cont>::reset() noexcept {

        destroy();

        contactDetector = ContactDetector<cont>(this);
        m_World->SetContactListener(&contactDetector.value());
        m_GameOver = false;
        m_PrevShaping = std::nullopt;

        std::vector<double> height = this->_np_random.uniform(0, H / 2, CHUNKS + 1), chunkX(CHUNKS), smoothY(CHUNKS);
        ...
        m_Moon = std::make_optional<box2d::DrawableBodyBase>();
        m_Moon->body = box2d::util::CreateStaticBody(*m_World, static_cast<b2Shape*>(&edgeShape));

        for (int j = 0; j < CHUNKS - 1; j++) {
            sf::Vector2f p1{static_cast<float>(chunkX[j]), static_cast<float>(smoothY[j])};
            sf::Vector2f p2{static_cast<float>(chunkX[j + 1]), static_cast<float>(smoothY[j + 1])};
        ...
        m_Lander = std::make_optional<box2d::DrawableBodyBase>();
        m_Lander->body = box2d::util::CreateDynamicBody(*m_World,
                                                  landerBodyDef,
                                                  box2d::util::FixtureDef(&landerShape, 5.0)
                                                  .friction(0.1)
                                                  .restitution(0.0)
                                                  .categoryBits(0x0010)
                                                  .maskBits(0x001).def);
        ...
        m_Lander->setUserData();

        m_Lander->body->ApplyForceToCenter({
            static_cast<float>(this->_np_random.template uniform(-INITIAL_RANDOM, INITIAL_RANDOM)),
            static_cast<float>(this->_np_random.template uniform(-INITIAL_RANDOM, INITIAL_RANDOM))}, true);
    ...
    template<bool cont>
    StepResponse< std::vector<double> > gym::LunarLandarEnv<cont>::step(const ActionT &_action) noexcept {
        ...
        auto tip = std::array<double, 2>{sin(m_Lander->body->GetAngle()), cos(m_Lander->body->GetAngle())};
        auto side = std::array<double, 2>{-tip[1], tip[0]};
        auto dispersion = this->_np_random.template uniform(-1, 1, 2) / SCALE;
        auto mPower = 0.f;
        ...
        if (power_condition_passed) {
            ...
            auto impulse_pos = b2Vec2{static_cast<float>(m_Lander->body->GetPosition().x + ox),
                                      static_cast<float>(m_Lander->body->GetPosition().y + oy)};
        ...
        if (power_condition_passed) {
            ...
            auto impulse_pos = b2Vec2{static_cast<double>(m_Lander->body->GetPosition().x + ox - tip[0] * 17 / SCALE),
                                      static_cast<double>(m_Lander->body->GetPosition().y + oy + tip[1] * SIDE_ENGINE_HEIGHT / SCALE)};

            auto p = createParticle(0.7, impulse_pos.x, impulse_pos.y, sPower);
            p->ApplyLinearImpulse(
                    {static_cast<float>(ox * SIDE_ENGINE_POWER * sPower),
                     static_cast<float>(oy * SIDE_ENGINE_POWER * sPower)},
                    impulse_pos,
                    true
            );

            m_Lander->body->ApplyLinearImpulse({static_cast<float>(-ox * SIDE_ENGINE_POWER * sPower),
                                               static_cast<float>(-oy * SIDE_ENGINE_POWER * sPower)},
                                              impulse_pos, true);
        }
