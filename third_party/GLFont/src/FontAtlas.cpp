...
//                       pixelSize); // Pixel height (0 defaults to pixel width)
    ...
    FT_Set_Char_Size(_face,      // Font face handle
                       0,          // Pixel width  (0 defaults to pixel height)
                       pixelSize*64,
                       96, 96); // Pixel height (0 defaults to pixel width)
    ...
    for(int i = 32; i < 128; ++i) {
        ...
        auto pad = std::string("0");
        glTexSubImage2D(GL_TEXTURE_2D, 0, texPos, 0, 1, _slot->bitmap.rows, GL_RED, GL_UNSIGNED_BYTE, pad.data()); // padding
