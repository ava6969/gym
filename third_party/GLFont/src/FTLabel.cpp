  ...
  _height(0)
{
    ...
    dir /= "sam/gym/third_party/GLFont/shaders";

    GLUtils::loadShader( (dir / "fontVertex.shader").c_str(), GL_VERTEX_SHADER, _programId);
    GLUtils::loadShader( (dir / "fontFragment.shader").c_str(), GL_FRAGMENT_SHADER, _programId);
...
void FTLabel::recalculateVertices(const char* text, float x, float y, int width, int height) {
    ...
    vector<string> words = splitText(_text.c_str());
...
void FTLabel::setText(std::string const&  text) {
    _text = text;
