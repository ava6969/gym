...
namespace sc2{
    ...
    struct StaticData{
        ...
        explicit StaticData(sc_pb::ResponseData const& data);

        [[nodiscard]] inline auto abilities() const { return m_abilities; }
        [[nodiscard]] inline auto upgrades() const { return m_upgrades; }
        [[nodiscard]] inline auto units() const { return m_units; }
        [[nodiscard]] inline auto unitStats() const { return m_unitStats; }
        [[nodiscard]] inline auto generalAbilities() const { return m_generalAbilities; }

    private:
        std::vector<uint32_t> m_unitStats, m_generalAbilities;
        std::unordered_map<uint32_t, std::string> m_units;
        std::unordered_map<uint32_t, sc_pb::UpgradeData> m_upgrades;
        std::unordered_map<uint32_t, sc_pb::AbilityData> m_abilities;
    };
